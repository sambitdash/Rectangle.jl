...
function countvalidity(t::RandomizedBinarySearchTree{K,V}) where {K,V}
    valid = true
    _inorder(t.root, t) do n
        !valid && return valid
        valid &= n.l.n + n.r.n + 1 == n.n
    end
end
...
@testset "Random BST" begin
    a = [4, 5, 3, 1, 10, 2, 7, 6, 8, 9]
    t = RandomizedBinarySearchTree{Int, Int}()
    ...
    s = RandomizedBinarySearchTree{Int, Int}()
    for i in b
       insert!(s, i, 10i)
    end
    bb = [i for i in 1:10]
    ...
    @testset "Non-unique" begin
        ...
        @test countvalidity(t)
        @test parentvalidity(t)
        @test bstvalidity(t)
        x = delete!(t, t.root.k)
        @assert t.root.n == 9999
        @test countvalidity(t)
        @test parentvalidity(t)
        @test bstvalidity(t)
        insert!(t, x.first, x.second)
        @test countvalidity(t)
        @test parentvalidity(t)
        @test bstvalidity(t)
        @assert t.root.n == 10000
        for i = 1:div(n, 2)
            delete!(t, a[i])
        end
        @test countvalidity(t)
...
@static if RUN_BENCHMARK
    ...
    begin
        ...
        print("Inserting random $n values into RandomizedBinarySearchTree: \t")
        @btime for i = 1:n
            insert!(t, a[i], i)
        end
        print("Base.find call to find a number in array: \t\t")
